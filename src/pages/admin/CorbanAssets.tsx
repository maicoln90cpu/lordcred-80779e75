...
export default function CorbanAssets() {
  ...
  const handleSync = async (assetType: string) => {
    ...
    // Parse response and upsert to cache
    ...
    if (items.length > 0) {
      ...
      toast.success(`${items.length} items sincronizados para ${assetType}`);
      refetch();
    } else {
      toast.info(`Nenhum item retornado para ${assetType}`);
    }
    setSyncing(null);
  };
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center justify-between">
                    <span>{t.label} ({cachedAssets.length} itens)</span>
