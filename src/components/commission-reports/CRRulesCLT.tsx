...
export default function CRRulesCLT() {
  ...
  const handleSave = async () => {
    if (!editing || !editing.banco) {
      toast({ title: 'Banco é obrigatório', variant: 'destructive' });
      return;
    }
    setSaving(true);
    try {
      const payload = {
        data_vigencia: editing.data_vigencia!,
        banco: editing.banco!,
        tabela_chave: editing.tabela_chave || '*',
        seguro: editing.seguro || 'Ambos',
        prazo_min: editing.prazo_min ?? 0,
        prazo_max: editing.prazo_max ?? 999,
        taxa: editing.taxa ?? 0,
      };
    ...
    <Card>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-xs">Seguro</TableHead>
                  <TableHead className="text-xs text-right">Prazo Mín</TableHead>
                  <TableHead className="text-xs text-right">Prazo Máx</TableHead>
                ...
                {filtered.map(r => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="text-xs">
                      <Badge variant={r.seguro === 'Sim' ? 'default' : r.seguro === 'Não' ? 'outline' : 'secondary'} className="text-[10px]">
                        {r.seguro}
                      </Badge>
                    </TableCell>
      ...
      <Dialog open={!!editing} onOpenChange={() => setEditing(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editing?.id ? 'Editar' : 'Nova'} Regra CLT</DialogTitle>
          </DialogHeader>
          {editing && (
            <div className="grid gap-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-muted-foreground">Vigência</label>
                  <Input type="date" value={editing.data_vigencia || ''} onChange={e => setEditing(p => ({ ...p!, data_vigencia: e.target.value }))} />
                </div>
                <div>
                  <label className="text-xs font-medium text-muted-foreground">Banco</label>
                  <Input value={editing.banco || ''} onChange={e => setEditing(p => ({ ...p!, banco: e.target.value.toUpperCase() }))} placeholder="Ex: HUB CREDITO" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-muted-foreground">Tabela/Chave</label>
                  <Input value={editing.tabela_chave || '*'} onChange={e => setEditing(p => ({ ...p!, tabela_chave: e.target.value }))} placeholder="* = todas" />
                </div>
                <div>
                  <label className="text-xs font-medium text-muted-foreground">Seguro</label>
                  <Select value={editing.seguro || 'Ambos'} onValueChange={v => setEditing(p => ({ ...p!, seguro: v }))}>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="text-xs font-medium text-muted-foreground">Prazo Mín</label>
                  <Input type="number" value={editing.prazo_min ?? 0} onChange={e => setEditing(p => ({ ...p!, prazo_min: parseInt(e.target.value) || 0 }))} />
                </div>
                <div>
                  <label className="text-xs font-medium text-muted-foreground">Prazo Máx</label>
                  <Input type="number" value={editing.prazo_max ?? 999} onChange={e => setEditing(p => ({ ...p!, prazo_max: parseInt(e.target.value) || 999 }))} />
                </div>
                <div>
                  <label className="text-xs font-medium text-muted-foreground">Taxa %</label>
                  <Input type="number" step="0.01" value={editing.taxa ?? 0} onChange={e => setEditing(p => ({ ...p!, taxa: parseFloat(e.target.value) || 0 }))} />
                </div>
              </div>
            </div>
          )}
      ...
      <Dialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">
            Excluir regra do banco <strong>{deleteTarget?.banco}</strong> com taxa <strong>{deleteTarget?.taxa}%</strong>?
          </p>
          <DialogFooter>
