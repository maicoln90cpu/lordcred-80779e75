...
import { Loader2, Calculator, Search, Download, ArrowUpDown, ArrowUp, ArrowDown } from 'lucide-react';
...
/** Identify product type from tipo_operacao/convenio */
...
/** Check if contract has insurance based on convenio/tabela name */
function hasInsurance(convenio: string | null): boolean {
  const c = (convenio || '').toUpperCase();
  return c.includes('SEGURO') || c.includes('COM SEG') || c.includes('C/SEG');
}

/** Extract table key for rule matching from convenio field */
function extractTableKey(convenio: string | null): string {
  return (convenio || '*').trim();
}

/** Find best matching FGTS rule */
function findFGTSRate(rules: RuleFGTS[], banco: string, valor: number, tabela: string, temSeguro: boolean, dataPgt: string | null): number {
  ...
  // Filter by banco and vigencia
  const candidates = rules
    .filter(r => r.banco.toUpperCase() === b && r.data_vigencia <= dt)
    .sort((a, b2) => b2.data_vigencia.localeCompare(a.data_vigencia));

  // Try specific tabela match first, then wildcard
  for (const r of candidates) {
    const tabelaMatch = r.tabela_chave === '*' || tabela.toUpperCase().includes(r.tabela_chave.toUpperCase());
    const seguroMatch = r.seguro === 'Ambos' || (r.seguro === 'Sim' && temSeguro) || (r.seguro === 'Não' && !temSeguro);
    const valorMatch = valor >= r.min_valor && valor <= r.max_valor;
    if (tabelaMatch && seguroMatch && valorMatch) return r.taxa;
  }
  return 0;
}

/** Find best matching CLT rule */
function findCLTRate(rules: RuleCLT[], banco: string, prazo: number, tabela: string, temSeguro: boolean, dataPgt: string | null): number {
  ...
  for (const r of candidates) {
    const tabelaMatch = r.tabela_chave === '*' || tabela.toUpperCase().includes(r.tabela_chave.toUpperCase());
    const seguroMatch = r.seguro === 'Ambos' || (r.seguro === 'Sim' && temSeguro) || (r.seguro === 'Não' && !temSeguro);
    const prazoMatch = prazo >= r.prazo_min && prazo <= r.prazo_max;
    if (tabelaMatch && seguroMatch && prazoMatch) return r.taxa;
  }
  return 0;
}
...
// ==================== COMPONENT ====================
interface CRRelatorioProps {
  divergenciasOnly?: boolean;
}
...
export default function CRRelatorio({ divergenciasOnly = false }: CRRelatorioProps) {
  ...
  const { data: geral = [], isLoading: l1 } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('cr_geral').select('id, ade, cod_contrato, cpf, nome_cliente, banco, prod_liq, prazo, tipo_operacao, convenio, cms_rep, data_pgt_cliente').limit(5000);
      return (data || []) as GeralRow[];
    },
  });
  ...
  const { data: repasse = [], isLoading: l2 } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('cr_repasse').select('id, ade, cod_contrato, cms_rep_favorecido').limit(5000);
  ...
  const { data: seguros = [], isLoading: l3 } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('cr_seguros').select('id, descricao, valor_comissao').limit(5000);
      return (data || []) as SeguroRow[];
    },
  });
  ...
  const { data: rulesFGTS = [], isLoading: l4 } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('cr_rules_fgts').select('*').order('data_vigencia', { ascending: false });
  ...
  const { data: rulesCLT = [], isLoading: l5 } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('cr_rules_clt').select('*').order('data_vigencia', { ascending: false });
  ...
  // Build lookup maps
  const repasseByAde = useMemo(() => {
    const map = new Map<string, number>();
    for (const r of repasse) {
      if (r.ade) {
        map.set(r.ade, (map.get(r.ade) || 0) + (r.cms_rep_favorecido || 0));
      }
    }
    return map;
  }, [repasse]);
  ...
  const seguroByAde = useMemo(() => {
    ...
    for (const s of seguros) {
      if (s.descricao) {
        // Try to find ADE reference in description
        const desc = s.descricao.toUpperCase();
        for (const g of geral) {
          if (g.ade && desc.includes(g.ade.toUpperCase())) {
            map.set(g.ade, (map.get(g.ade) || 0) + (s.valor_comissao || 0));
          }
        }
      }
    }
  ...
  const reportRows: ReportRow[] = useMemo(() => {
    return geral.map(g => {
      ...
      const temSeguro = hasInsurance(g.convenio);
      const tabela = extractTableKey(g.convenio);

      // Mercantil: adjust value
      const valorCalc = banco === 'MERCANTIL' ? valor / 0.7 : valor;

      // Comissão recebida = geral + repasse + seguro
      const cmsGeral = g.cms_rep || 0;
      const cmsRepasse = repasseByAde.get(ade) || 0;
      const cmsSeguro = seguroByAde.get(ade) || 0;
      const recebida = cmsGeral + cmsRepasse + cmsSeguro;

      // Comissão esperada
      let esperada = 0;
      if (produto === 'FGTS') {
        const taxa = findFGTSRate(rulesFGTS, banco, valorCalc, tabela, temSeguro, g.data_pgt_cliente);
        esperada = Math.round(valorCalc * taxa / 100 * 100) / 100;
      } else {
        const taxa = findCLTRate(rulesCLT, banco, prazo, tabela, temSeguro, g.data_pgt_cliente);
        esperada = Math.round(valorCalc * taxa / 100 * 100) / 100;
      }
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      rows = rows.filter(r =>
        r.ade.toLowerCase().includes(q) ||
        r.nome.toLowerCase().includes(q) ||
        r.cpf.includes(q) ||
        r.cod_contrato.toLowerCase().includes(q)
      );
    }
    return rows;
  ...
  const handleExport = () => {
    const ws = XLSX.utils.json_to_sheet(sorted.map(r => ({
      ADE: r.ade,
      Contrato: r.cod_contrato,
      CPF: r.cpf,
      Nome: r.nome,
      Banco: r.banco,
      Produto: r.produto,
      'Valor Lib.': r.valor_liberado,
      Prazo: r.prazo,
      'CMS Geral': r.cms_geral,
      'CMS Repasse': r.cms_repasse,
      'CMS Seguro': r.cms_seguro,
      'Recebida': r.comissao_recebida,
      'Esperada': r.comissao_esperada,
  ...
  const desc = divergenciasOnly
    ...
    : 'Cruzamento automático: Geral + Repasse + Seguros + Regras → Comissão Esperada vs Recebida.';
    ...
    <Card>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Badge variant="outline" className="text-xs">
              {totals.count} contratos
            </Badge>
            <Badge variant="secondary" className="text-xs">
              Recebida: {fmtBRL(totals.recebida)}
            </Badge>
            <Badge variant="secondary" className="text-xs">
              Esperada: {fmtBRL(totals.esperada)}
            </Badge>
            <Badge variant={totals.diferenca >= 0 ? 'default' : 'destructive'} className="text-xs">
              Δ {fmtBRL(totals.diferenca)}
            </Badge>
          </div>
        </div>
        <div className="flex items-center gap-2 pt-2">
          <div className="relative">
            ...
            <Input placeholder="Buscar ADE, nome, CPF..." value={search} onChange={e => setSearch(e.target.value)} className="pl-8 w-56 h-9" />
          </div>
          <select
            value={filterBanco}
      ...
      <CardContent>
        {isLoading ? (
          ...
          <p className="text-center text-muted-foreground py-8 text-sm">
            {divergenciasOnly ? 'Nenhuma divergência encontrada! 🎉' : 'Nenhum resultado para os filtros aplicados.'}
          </p>
        ) : (
          <div className="border rounded-lg max-h-[600px] overflow-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <SortHead label="ADE" sortKey="ade" sort={sort} toggle={toggleSort} className="text-xs whitespace-nowrap" />
                  <SortHead label="Nome" sortKey="nome" sort={sort} toggle={toggleSort} className="text-xs whitespace-nowrap" />
                  <SortHead label="Banco" sortKey="banco" sort={sort} toggle={toggleSort} className="text-xs whitespace-nowrap" />
                  <SortHead label="Produto" sortKey="produto" sort={sort} toggle={toggleSort} className="text-xs whitespace-nowrap" />
                  <SortHead label="Valor Lib." sortKey="valor_liberado" sort={sort} toggle={toggleSort} className="text-xs whitespace-nowrap text-right" />
                  <TableHead className="text-xs text-right whitespace-nowrap">CMS Geral</TableHead>
                  <TableHead className="text-xs text-right whitespace-nowrap">CMS Rep.</TableHead>
                  <TableHead className="text-xs text-right whitespace-nowrap">CMS Seg.</TableHead>
                  <SortHead label="Recebida" sortKey="comissao_recebida" sort={sort} toggle={toggleSort} className="text-xs whitespace-nowrap text-right" />
                  <SortHead label="Esperada" sortKey="comissao_esperada" sort={sort} toggle={toggleSort} className="text-xs whitespace-nowrap text-right" />
                  <SortHead label="Diferença" sortKey="diferenca" sort={sort} toggle={toggleSort} className="text-xs whitespace-nowrap text-right" />
                </TableRow>
              </TableHeader>
              <TableBody>
                {sorted.slice(0, 500).map((r, i) => (
                  <TableRow key={i} className={Math.abs(r.diferenca) > 0.01 ? 'bg-destructive/5' : ''}>
                    ...
                    <TableCell className="text-xs">
                      <Badge variant={r.produto === 'FGTS' ? 'default' : 'secondary'} className="text-[10px]">{r.produto}</Badge>
                    </TableCell>
                    ...
                    <TableCell className={`text-xs text-right font-mono font-bold ${r.diferenca > 0.01 ? 'text-green-600' : r.diferenca < -0.01 ? 'text-destructive' : ''}`}>
                      {fmtBRL(r.diferenca)}
                    </TableCell>
                  </TableRow>
                ))}
            ...
            {sorted.length > 500 && (
              <p className="text-center text-xs text-muted-foreground py-2">Mostrando 500 de {sorted.length}. Use filtros para refinar.</p>
            )}
          </div>
        )}
      </CardContent>
