...
export default function CRHistorico() {
  ...
  const { data: detalhes = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('cr_historico_detalhado')
        .select('*')
        .eq('gestao_id', expanded!)
        .order('banco')
        .limit(2000);
      if (error) throw error;
      return data as Detalhe[];
    },
  });
      ...
      <CardContent>
        {isLoading ? (
          ...
          <p className="text-center text-muted-foreground py-8 text-sm">
            Nenhum fechamento salvo. Use a aba Resumo para salvar um fechamento.
          </p>
        ) : (
                    ...
                    <div className="flex items-center justify-between p-4">
                          ...
                          <div className="flex items-center gap-2 mt-1 flex-wrap">
                            ...
                            {g.data_inicio && g.data_fim && (
                              <span className="text-xs text-muted-foreground">
                                {new Date(g.data_inicio).toLocaleDateString('pt-BR')} — {new Date(g.data_fim).toLocaleDateString('pt-BR')}
                              </span>
                            )}
                            <span className="text-xs text-muted-foreground">
                              Salvo em {new Date(g.created_at).toLocaleDateString('pt-BR')}
                            </span>
                          </div>
                        </div>
                      </CollapsibleTrigger>
                      <div className="flex items-center gap-4">
                        <div className="text-right text-sm">
                          ...
                          <p className={`font-bold font-mono ${dif > 0.01 ? 'text-green-600' : dif < -0.01 ? 'text-destructive' : ''}`}>
                            Δ {fmtBRL(dif)}
                          </p>
                        </div>
                            ...
                            <Table>
                              <TableHeader>
                                <TableRow>
                                  ...
                                  <TableHead className="text-xs text-right">Valor Lib.</TableHead>
                                  <TableHead className="text-xs text-right">Recebida</TableHead>
                                ...
                                {detalhes.map(d => {
                                  ...
                                  return (
                                    <TableRow key={d.id} className={Math.abs(dd) > 0.01 ? 'bg-destructive/5' : ''}>
                                      ...
                                      <TableCell className={`text-xs text-right font-mono font-bold ${dd > 0.01 ? 'text-green-600' : dd < -0.01 ? 'text-destructive' : ''}`}>
                                        {fmtBRL(dd)}
                                      </TableCell>
                                    </TableRow>
                                  );
      ...
      <Dialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">
            Excluir <strong>"{deleteTarget?.nome}"</strong> e todos os <strong>{deleteTarget?.qtd_propostas}</strong> detalhes associados?
          </p>
          <DialogFooter>
