...
import { exportPixToXlsx, downloadPixTemplate, importPixFromFile } from './pixImportExport';
...
export default function PixTab({ profiles, getSellerName, isAdmin, userId }: PixTabProps) {
  ...
  const handleExport = async () => {
    ...
    await exportPixToXlsx(visiblePix, getSellerName, 'chaves_pix.xlsx');
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImporting(true);
    try {
      const res = await importPixFromFile(file, profiles, 'seller_pix');
