-- ============================================================
-- ETAPA 2 — Correções V2 (FGTS fallback, meses, case-insensitive,
--          ELSIFs novos, recálculo, seed annual_rewards_v2)
-- ============================================================

-- 1) Padronizar prazos FGTS V2 em MESES.
-- Marcador: registros existentes com term_max <= 50 são considerados "anos"
-- e convertidos para meses (×12). Valores já em meses (term_max > 50) são preservados.
UPDATE public.commission_rates_fgts_v2
SET term_min = term_min * 12,
    term_max = term_max * 12,
    obs = COALESCE(obs || ' | ', '') || 'auto-convertido anos→meses 2026-05'
WHERE term_max <= 50;

-- 2) Seed commission_annual_rewards_v2 a partir de V1 SE estiver vazio
INSERT INTO public.commission_annual_rewards_v2 (min_contracts, reward_description, sort_order)
SELECT min_contracts, reward_description, sort_order
FROM public.commission_annual_rewards
WHERE NOT EXISTS (SELECT 1 FROM public.commission_annual_rewards_v2);

-- 3) Atualizar trigger V2 com:
--    - UPPER(bank) em todos SELECTs (case-insensitive)
--    - Fallback FGTS (somente bank+insurance+date) quando específico/genérico não casam
--    - ELSIFs ampliados: PARANA TURBO, FACTA SAQUE, FACTA NOVO, FACTA AGENDADO
...
BEGIN
  ...
  -- Heurísticas de table_key (ampliadas)
  ...
  IF NEW.product = 'FGTS' THEN
    -- (a) Específico por table_key + prazo + valor
    ...
    -- (b) Genérico: table_key NULL + prazo + valor
    ...
    -- (c) FALLBACK simples (paridade V1): bank + insurance + date, ignorando prazo/valor/tabela
    ...
    _rate := COALESCE(NULLIF(_rate_specific, 0), NULLIF(_rate_generic, 0), _rate_fallback, 0);
...
-- 4) Função para recalcular TODAS as vendas V2 (botão na ConfigTab)
...
BEGIN
  ...
  -- Touch every row para disparar a trigger BEFORE UPDATE
