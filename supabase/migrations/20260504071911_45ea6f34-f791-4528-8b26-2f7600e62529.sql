-- ============================================================================
-- Comissões V2 — Etapa 1+2: Normalização de banco, conversão FGTS anos→meses,
-- fallback CLT por banco+seguro+data, e RPC com contagens de match level.
-- ============================================================================
...
BEGIN
  ...
  -- Normalização de aliases de banco (lista de nomes equivalentes para tentar match)
  _bank_aliases := ARRAY[_bank_upper];
  IF _bank_upper LIKE '%LOTUS%' THEN
    _bank_aliases := _bank_aliases || ARRAY['LOTUS', 'LOTUS MAIS'];
  ELSIF _bank_upper LIKE '%HUB%' THEN
    _bank_aliases := _bank_aliases || ARRAY['HUB', 'HUB CREDITO', 'HUB CRÉDITO'];
  ELSIF _bank_upper LIKE '%V8%' THEN
    _bank_aliases := _bank_aliases || ARRAY['V8', 'V8 BANK'];
  END IF;
  -- de-duplica
  ...
  -- FGTS: prazos 1..5 são interpretados como anos → converter para meses
  ...
  IF NEW.product = 'FGTS' THEN
    IF _table_key IS NOT NULL THEN
      ...
      WHERE UPPER(TRIM(bank)) = ANY(_bank_aliases)
        ...
        AND term_min <= _term_match
        AND term_max >= _term_match
  ...
  ELSIF NEW.product = 'Crédito do Trabalhador' THEN
    ...
    -- NOVO: fallback CLT por banco+seguro+data (paridade V1)
    SELECT rate INTO _rate_fallback
    FROM commission_rates_clt_v2
    WHERE UPPER(TRIM(bank)) = ANY(_bank_aliases)
      AND (CASE WHEN _is_hub THEN true ELSE has_insurance = NEW.has_insurance END)
...
-- RPC com contagens de match level
...
BEGIN
  ...
  SELECT
    COUNT(*) FILTER (WHERE rate_match_level = 'specific'),
    COUNT(*) FILTER (WHERE rate_match_level = 'generic'),
    COUNT(*) FILTER (WHERE rate_match_level = 'fallback'),
    COUNT(*) FILTER (WHERE rate_match_level = 'none' OR rate_match_level IS NULL)
  INTO _specific, _generic, _fallback, _none
  FROM public.commission_sales_v2;
