...
BEGIN
  ...
  _bank_upper := UPPER(TRIM(COALESCE(NEW.bank, '')));
  -- Normalizado sem acento para casar PARANÁ/PARANA, PRESENÇA/PRESENCA, CRÉDITO/CREDITO
  _bank_norm := TRANSLATE(_bank_upper,
    'ÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇ',
    'AAAAAEEEEIIIIOOOOOUUUUC');
  _is_hub := (_bank_norm LIKE '%HUB%');

  _bank_aliases := ARRAY[_bank_upper, _bank_norm];

  -- Famílias já suportadas
  IF _bank_norm LIKE '%LOTUS%' THEN
    _bank_aliases := _bank_aliases || ARRAY['LOTUS', 'LOTUS MAIS', 'LOTUS+'];
  ELSIF _bank_norm LIKE '%HUB%' THEN
    _bank_aliases := _bank_aliases || ARRAY['HUB', 'HUB CREDITO', 'HUB CRÉDITO', 'HUBCREDITO'];
  ELSIF _bank_norm LIKE '%V8%' THEN
    _bank_aliases := _bank_aliases || ARRAY['V8', 'V8 BANK', 'V8BANK'];

  -- Novas famílias (Etapa aliases)
  ELSIF _bank_norm LIKE '%C6%' THEN
    _bank_aliases := _bank_aliases || ARRAY['C6', 'BANCO C6', 'BANCOC6'];
  ELSIF _bank_norm LIKE '%MERCANTIL%' THEN
    _bank_aliases := _bank_aliases || ARRAY['MERCANTIL', 'MERCANTIL DO BRASIL', 'BANCO MERCANTIL'];
  ELSIF _bank_norm LIKE '%PRESENCA%' THEN
    _bank_aliases := _bank_aliases || ARRAY['PRESENCA', 'PRESENÇA', 'PRESENCA BANK', 'PRESENÇA BANK', 'BANCO PRESENCA', 'BANCO PRESENÇA'];
  ELSIF _bank_norm LIKE '%PRATA%' OR _bank_norm LIKE '%CELCOIN%' OR _bank_norm LIKE '%QI TECH%' OR _bank_norm LIKE '%QITECH%' THEN
    _bank_aliases := _bank_aliases || ARRAY['PRATA', 'PRATA DIGITAL', 'PRATA CELCOIN', 'CELCOIN', 'QI TECH', 'QITECH'];
  ELSIF _bank_norm LIKE '%HAPPY%' THEN
    _bank_aliases := _bank_aliases || ARRAY['HAPPY', 'HAPPY CONSIG', 'HAPPYCONSIG'];
  ELSIF _bank_norm LIKE '%ZILI%' THEN
    _bank_aliases := _bank_aliases || ARRAY['ZILICRED', 'ZILI CRED'];
  ELSIF _bank_norm LIKE '%QUALI%' THEN
    _bank_aliases := _bank_aliases || ARRAY['QUALIBANK', 'QUALI BANK'];
  ELSIF _bank_norm LIKE '%FACTA%' THEN
    _bank_aliases := _bank_aliases || ARRAY['FACTA', 'BANCO FACTA'];
  ELSIF _bank_norm LIKE '%PARANA%' THEN
    _bank_aliases := _bank_aliases || ARRAY['PARANA BANCO', 'PARANÁ BANCO', 'PARANA', 'PARANÁ'];
  END IF;
  _bank_aliases := ARRAY(SELECT DISTINCT unnest(_bank_aliases));
  ...
  _term_match := COALESCE(NEW.term, 0);
  IF NEW.product = 'FGTS' AND _term_match BETWEEN 1 AND 5 THEN
    _term_match := _term_match * 12;
  END IF;
  ...
  IF NEW.product = 'FGTS' THEN
    IF _table_key IS NOT NULL THEN
      ...
      WHERE UPPER(TRIM(bank)) = ANY(_bank_aliases)
        AND table_key = _table_key
        AND has_insurance = NEW.has_insurance
        AND term_min <= _term_match AND term_max >= _term_match
    ...
    SELECT rate INTO _rate_generic_nv
    FROM commission_rates_fgts_v2
    WHERE UPPER(TRIM(bank)) = ANY(_bank_aliases)
      AND has_insurance = NEW.has_insurance
      AND term_min <= _term_match AND term_max >= _term_match
    ...
    ELSIF COALESCE(_rate_generic_nv, 0) > 0 THEN
      _rate := _rate_generic_nv; _match_level := 'generic_no_value';
  ...
  ELSIF NEW.product = 'Crédito do Trabalhador' THEN
    IF _table_key IS NOT NULL THEN
      ...
      WHERE UPPER(TRIM(bank)) = ANY(_bank_aliases)
        ...
        AND term_min <= COALESCE(NEW.term, 0) AND term_max >= COALESCE(NEW.term, 0)
        AND min_value <= COALESCE(NEW.released_value, 0)
        AND max_value >= COALESCE(NEW.released_value, 0)
        AND effective_date <= _sale_date
      ORDER BY effective_date DESC LIMIT 1;
  ...
  IF _bonus_threshold IS NOT NULL AND _bonus_threshold > 0 THEN
    ...
    ELSE
      ...
      WHERE seller_id = NEW.seller_id
        AND week_label = NEW.week_label
        AND id != COALESCE(NEW.id, '00000000-0000-0000-0000-000000000000'::uuid);
      _weekly_total := _weekly_total + COALESCE(NEW.released_value, 0);
