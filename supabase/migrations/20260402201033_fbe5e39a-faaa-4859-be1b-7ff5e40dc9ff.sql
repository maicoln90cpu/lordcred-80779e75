-- Add new columns to commission_settings
ALTER TABLE public.commission_settings 
  ADD COLUMN IF NOT EXISTS bonus_mode text NOT NULL DEFAULT 'valor',
  ADD COLUMN IF NOT EXISTS bonus_fixed_value numeric NOT NULL DEFAULT 0;

-- Update the calculate_commission trigger function
...
BEGIN
  ...
  IF _bonus_threshold IS NOT NULL AND _bonus_threshold > 0 THEN
    IF _bonus_mode = 'contratos' THEN
      -- Count contracts in the same week
    ...
    ELSE
      -- Original mode: sum released_value
